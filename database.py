    ...
    def create_task_table(self):
        self.cursor.execute("""
            CREATE TABLE IF NOT EXISTS tasks(
                ...
                category varchar(50))""")  # Ensure the category column is added

    def create_task(self, task, due_date=None, category='General'):
        self.cursor.execute("""
            INSERT INTO tasks(task, due_date, completed, category) 
            VALUES(?, ?, ?, ?)""", (task, due_date, 0, category))
        self.con.commit()
        created_task = self.cursor.execute("""
            SELECT id, task, due_date, category 
            FROM tasks WHERE task = ? and completed = 0""", (task,)).fetchall()
